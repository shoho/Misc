...
`tailwind.config` やテーマファイル、既存コンポーネントのスタイルを読み取り、次の3ファイルを生成します:

- `designs/tokens.css` — 全トークンの CSS カスタムプロパティ定義(両テーマ)。トークン名は固定の命名契約(`--bg` / `--ink` / `--accent` / `--space-*` など)に従うので、以降のワークフローが確実に参照できます
- `designs/design-system.md` — トークン一覧・書体の意図・コンポーネント基本形・Do/Don't
- `designs/design-system/index.html` — 全トークンを一覧できるスタイルガイド(検証用)
...
- **トークン名を固定契約に**:`/design-system` が生成する名前を全ワークフローが前提にするので、モデルが名前を発明してズレない
- **判断を機械的な規則に**:値が矛盾したときの優先順位(ユーザー指定 > コード > 画像推定)、曖昧なフィードバックの翻訳表(「重い」→ 余白/罫線/ウェイトを疑う)
